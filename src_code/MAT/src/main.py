...
from utils.builder import build_model,build_datacollator

def training(config,device,data): 
    collator = build_datacollator(config)
...
def main(config_path: Text) -> None:
    ...
    training(config,device,data) #if you want predict comment this line and fix the link of inference checkpoint to the best model
